...
public abstract class BookmarkParserBase
{
    ...
    {
        if (folder.Name.Equals(name, StringComparison.OrdinalIgnoreCase)) return folder;
    ...
    protected List<BookmarkFolder> ProcessFolder(BookmarkFolder folder, Func<Bookmark, bool> filterPredicate,
        Func<Bookmark, Bookmark> transformFunc)
    {
        var result = new List<BookmarkFolder>();

        // Process current folder
        var processedFolder = ProcessSingleFolder(folder, filterPredicate, transformFunc);
        if (processedFolder.Bookmarks.Count > 0) result.Add(processedFolder);
    ...
    private BookmarkFolder ProcessSingleFolder(BookmarkFolder folder, Func<Bookmark, bool> filterPredicate,
        Func<Bookmark, Bookmark> transformFunc)
    {
